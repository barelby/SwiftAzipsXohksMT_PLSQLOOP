create or replace procedure copy_from_ibsres(p_date DATE DEFAULT NULL) is
  l_cur_id  integer;
  l_context ibs.context_value%rowtype;
  l_sername VARCHAR(100);
  l_date    DATE DEFAULT nvl(p_date, TRUNC(SYSDATE));
begin
  l_sername := SYS_CONTEXT('USERENV','SERVER_HOST');
  
  IF l_sername = 'orasrv2' THEN
      raise_application_error(-20000, 'Копирование на резерв нужно запускать с сервера отличного резерва!');
  END IF;
  
  dbms_output.put_line('CURRENT server ' || l_sername);
  ...
  LOOP
      BEGIN
          ...
          FROM ipay.v_interbankpayments_attrscol@ibs_res v
          ...
          FROM ipay.v_interbankpayments_statescol@ibs_res v
          WHERE v.payment_id = recur.id;
          -- Coping changes
          ...
          FROM ipay.v_interbankpayments_changescol@ibs_res v
          WHERE v.payment_id = recur.id;
          ...
          when not matched then insert ( id,
                                        ...
                                        changes,
                                        creation_date) 
                               values ( ipay.interbankspayments_seq.nextval,
                                        recur.reference,
                                        recur.state,
                                        recur.payment_date,
                                        recur.system_id,
         ...
         when matched then update set   state                       = recur.state,
            ...
            SELECT i.id into l_cur_id
            FROM ipay.interbankpayments i 
            WHERE i.reference = recur.reference;
            
            -- Update fee collection
            dbms_output.put_line('ID ' || l_cur_id);

            BEGIN
                l_context := ibs.api_context.read_context_value@ibs_res(recur.context_id);
